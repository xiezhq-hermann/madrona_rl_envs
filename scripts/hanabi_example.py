from envs.hanabi_env import HanabiMadrona, PantheonHanabi, validate_step
import torch
import time
...
for iter in tqdm(range(args.num_steps), desc="Running Simulation"):
    ...
    if args.validation and not validate_step(old_state, action, next_done, next_state, reward, STATIC_ENV, verbose=args.verbose):
        num_errors += 1
